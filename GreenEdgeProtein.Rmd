...
for(i in 1:nrow(Nested)) {
  # get data and model from current row
  # need to extract with [[]] since in a list
...
LongProteinData <- ProteinData %>%
  ...
  # subset columns
  select(SampleName, "Protein_ug_uL" = pred, PlateBaseName) %>%
...
WideProteinData <- LongProteinData %>%
  ...
  select(SampleName, TotalProtRep1 = "0", 
         ...
         AvgTotalProt, CV, StdDev, PlateBaseName)
...
BlotQuantData <- BlotQuantData %>%
  ...
  # merge columns with existing, manually entered total protein data
  unite("Rep1TotalProteinExtract_ug_uL", 
        Rep1TotalProteinExtract_ug_uL,
        TotalProtRep1,
        remove = TRUE, na.rm = TRUE) %>%
  unite("Rep2TotalProteinExtract_ug_uL", 
        Rep2TotalProteinExtract_ug_uL,
        TotalProtRep2,
        remove = TRUE, na.rm = TRUE) %>%
  unite("Rep3TotalProteinExtract_ug_uL", 
        Rep3TotalProteinExtract_ug_uL,
        TotalProtRep3,
        remove = TRUE, na.rm = TRUE) %>%
  type_convert() %>%
  mutate(AverageTotalProteinExtract_ug_uL = coalesce(AvgTotalProt,
                                                     AverageTotalProteinExtract_ug_uL)
         )%>%
  unite("TotalProteinExtractFileName", 
...
BlotQuantData %>%
  ...
  select(Sample, TotalProteinLoaded_ug, ManualTotalProteinLoaded_ug)
```
...
```{r}
BlotQuantData %>%
  filter(Sample %in% c("P357", "P376", "P379", "P382")) %>%
  select(Sample, TotalChla_ug, TotalChlaDany_ug, 
         ChlaLoaded_ug)
```
...
```
```{r  Getting error- Mireille Tagged}
#Fcp6PsbAData <- TargetProteinReport %>%
#  group_by(Sample, Target) %>%
#  pivot_wider(id_cols = Sample, names_from = Target, values_from = fmol_ugTotalProtein) %>%
 # mutate(PsbA_Fcp6 = PsbA / Fcp6) %>%
 # select(Sample, PsbA_Fcp6)
...
```{r  Getting error- Mireille Tagged}
#TargetProteinReport %>%
#  left_join(., Fcp6PsbAData, by = "Sample") %>%
#  filter(Exp != "Nf_Growth_vs.IceCamp",
#         PsbA_Fcp6 > 100) %>%
 # ggplot(.) +
 # geom_point(aes(x = TotalChlaExtract_ug, y = PsbA_Fcp6, colour = CultureId, shape = Sp))
```
MIrielle removed shape from wrap as there are too many now
```{r plot protein quant vs. experimental conditions}
# List target proteins, which conditions to plot, protein normalize methods
Targets <- c("Fcp6", "PsbA", "FtsH" )
...
```{r no natalie, frag, forced int }
lm(TotalProteinExtract_ug ~ 0 + TotalChlaExtract_ug, data = Data)
```

```{r with natalie forced int }
lm(TotalProteinExtract_ug ~ 0 + TotalChlaExtract_ug, data = Data)
```

```{r no natalie forced int }
lm(TotalProteinExtract_ug ~ 0 + TotalChlaExtract_ug, data = Data)
```
```{r no natalie, frag, unforced int }
lm(TotalProteinExtract_ug ~ TotalChlaExtract_ug, data = Data)
```

```{r with Natalie unforced int}
lm(TotalProteinExtract_ug ~ TotalChlaExtract_ug, data = Data)
```
```{r no Natalie unforced int}
lm(TotalProteinExtract_ug ~ TotalChlaExtract_ug, data = Data)
```

```{r OLD plot protein quant vs. experimental conditions}
# # List target proteins, which conditions to plot, protein normalize methods
# Targets <- c("Fcp6", "PsbA")
# Conditions <- c("Stress_h", "Stress_h")
# NormalizeMethods <- c("fmol_mL", "fmol_ugChla", "fmol_ugTotalProtein")
# 
# 
# # initialize lists, counter
...
#       
#       geom_point(aes(x= !!Condition, y= !!NormalizeMethod, 
#                  colour = Inhibitor, shape = as.factor(BlotId))) +
#       geom_vline(xintercept = 2, color = "red", size = 0.25) +
#   # facet wrap if stress_h is on x axis
#       {if(Conditions[i] == "Stress_h")facet_wrap(vars(CultureId))}+
#       theme_bw() +
